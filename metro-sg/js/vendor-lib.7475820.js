...
*/
    ...
    e.exports = function e(p) {
      return new Promise(function e(o, i) {
        ...
        if (p.auth) {
          ...
          var f = p.auth.password || '';
          u.Authorization = 'Basic ' + x(r + ':' + f);
        }
        s.open(p.method.toUpperCase(), m(p.url, p.params, p.paramsSerializer), true);
        s.timeout = p.timeout;
        s[t] = function e() {
          if (!s || (s.readyState !== 4 && !a)) {
            return;
          }
          if (s.status === 0 && !(s.responseURL && s.responseURL.indexOf('file:') === 0)) {
            return;
          }
          var t = 'getAllResponseHeaders' in s ? y(s.getAllResponseHeaders()) : null;
          var r = !p.responseType || p.responseType === 'text' ? s.responseText : s.response;
          var n = {
            data: r,
            status: s.status === 1223 ? 204 : s.status,
            statusText: s.status === 1223 ? 'No Content' : s.statusText,
            headers: t,
            config: p,
            request: s,
          };
        ...
        s.onerror = function e() {
          i(w('Network Error', p, null, s));
          s = null;
        };
        s.ontimeout = function e() {
          i(w('timeout of ' + p.timeout + 'ms exceeded', p, 'ECONNABORTED', s));
          s = null;
        };
        if (h.isStandardBrowserEnv()) {
          var c = d(87);
          var l =
            (p.withCredentials || g(p.url)) && p.xsrfCookieName
              ? c.read(p.xsrfCookieName)
              : undefined;
          if (l) {
            u[p.xsrfHeaderName] = l;
          }
        }
        if ('setRequestHeader' in s) {
          h.forEach(u, function e(t, r) {
        ...
        if (typeof p.onDownloadProgress === 'function') {
          s.addEventListener('progress', p.onDownloadProgress);
        }
        if (typeof p.onUploadProgress === 'function' && s.upload) {
          s.upload.addEventListener('progress', p.onUploadProgress);
        }
        if (p.cancelToken) {
          p.cancelToken.promise.then(function e(t) {
            if (!s) {
              return;
            }
            s.abort();
  ...
  83: function(e, t, r) {
    ...
    e.exports = function e(t, r, n) {
      ...
      if (n) {
        o = n(r);
      } else if (u.isURLSearchParams(r)) {
        o = r.toString();
      } else {
        var i = [];
        u.forEach(r, function e(t, r) {
  ...
  84: function(e, t, r) {
    ...
    e.exports = function e(t) {
      ...
      u.forEach(t.split('\n'), function e(t) {
        i = t.indexOf(':');
        n = u.trim(t.substr(0, i)).toLowerCase();
        o = u.trim(t.substr(i + 1));
        if (n) {
          if (r[n] && s.indexOf(n) >= 0) {
            return;
          }
          if (n === 'set-cookie') {
